  ...
  return (
    ...
    >
      <div className="max-w-[90%] mx-auto px-3 xs:px-4 sm:px-6 lg:px-8">
        ...
        {/* Company Description */}
        <div className={`max-w-4xl mx-auto mb-16 md:mb-24 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <div className="prose prose-lg max-w-none">
            {ABOUT_DESCRIPTION.split('\n\n').map((paragraph, index) => (
              <p
                key={index}
                className="text-base md:text-lg text-gray-dark leading-relaxed mb-6 last:mb-0"
              >
                {paragraph}
              </p>
            ))}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 md:gap-10">
          {LEADERSHIP_TEAM.map((member, index) => (
            ...
            >
              {/* Card Container */}
              <div className="bg-white rounded-2xl overflow-hidden border border-gray-200 hover:border-secondary transition-all duration-300 hover:shadow-lg">
                {/* Image Container */}
                ...
                <div className="p-6 md:p-8">
                  <h4 className="h4 text-dark mb-1">
                    {member.name}
                  </h4>
                ...
                {/* Bottom accent */}
                <div className="h-1 bg-linear-to-r from-primary via-secondary to-tertiary opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
              </div>
            </div>
          ))}
        </div>
        ...
        {/* Core Values Section */}
        <div className="mt-16 md:mt-24 pt-16 md:pt-24 border-t border-gray-200">
          <h3 className="h3 text-dark text-center mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div
                key={index}
                className={`text-center p-6 md:p-8 bg-gradient-soft rounded-xl border border-gray-200 hover:border-primary transition-all duration-300 hover:shadow-md ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                  }`}
                style={{
